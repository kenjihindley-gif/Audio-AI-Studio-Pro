...
async function createEngine(tabId, streamId) {
    // CORREÇÃO: Se o motor já existe, força o envio do estado atual para reativar o aspeto visual do Popup!
    if (tabEngines[tabId]) {
        calculateAndSendGraphData(tabId);
        const eng = tabEngines[tabId];
        chrome.runtime.sendMessage({ action: 'sync_popup_state', points: eng.points, boosterVolume: eng.boosterVolume, routing: eng.routing, tabId: tabId });
        return;
    }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    switch (message.action) {
        ...
        case 'set_routing_params': 
            ...
            updateRoutingState(engine); sendResponse({ status: "ok" }); break;
